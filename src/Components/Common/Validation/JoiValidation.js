...
export const validateProperty = (
  ...
  if (required) {
    Joischema = {
      ...
      password: Joi.string().min(4).alphanum().required().error(() => {
        return { message: `Required Proper Password` };
      }),
      email: Joi.string().email().required().error(() => {
        return { message: `Required valid Mail Id` };
      }),
    ...
    Joischema = {
      ...
      password: Joi.string().min(4).alphanum().label(label).allow(null),
      contact: Joi.number()
        .integer()
        .min(1000000000)
        .max(9999999999)
